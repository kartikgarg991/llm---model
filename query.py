...
def rewrite_query(history, query):
    ...
    rewrite the current query such that it is standalone and self-contained and doe snot require the past conversation .
...
# # --- Streamlit / custom thread me event loop fix ---
# try:
...
# # Streamlit UI
# st.set_page_config(page_title="PDF QnA", page_icon="📑")
# st.title("📑 PDF Question Answering (RAG with Pinecone + Gemini)")
...
# if query:
#     # Combine last 2 history turns + query
#     search_query = " ".join(st.session_state.conversation_history[-2:]) + " " + query
#     queryVector = embeddings.embed_query(search_query)
...
#         If not relevant, say 'No relevant content found in the document.'
#         """
